...
! *** Module internal variables
  ...
  TYPE obs_l
     ...
     REAL(kind=8), ALLOCATABLE :: distance_l(:)   !< Distances of local observations
     REAL(kind=8), ALLOCATABLE :: ivar_obs_l(:)   !< Inverse variance of local observations
     INTEGER :: locweight                 !< Specify localization function
     REAL(kind=8) :: lradius                      !< localization radius
     REAL(kind=8) :: sradius                      !< support radius for localization function
  END TYPE obs_l
...
! *** Arguments ***
    ...
    REAL(kind=8), INTENT(in) :: coords_l(:)          !< Coordinates of current analysis domain
    INTEGER, INTENT(in) :: locweight         !< Type of localization function
    REAL(kind=8), INTENT(in) :: lradius              !< Localization radius
    REAL(kind=8), INTENT(in) :: sradius              !< Support radius of localization function
...
! *** Arguments ***
    ...
    TYPE(obs_f), INTENT(inout) :: thisobs    !< Data type with full observation
    REAL(kind=8), INTENT(in) :: coords_l(:)          !< Coordinates of current analysis domain (thisobs%ncoord)
    REAL(kind=8), INTENT(in) :: lradius              !< Localization radius in meters

! *** Local variables ***
    INTEGER :: i            ! Counters
    REAL(kind=8) :: ocoord(thisobs%ncoord)  ! Coordinates of observation
...
! *** Arguments ***
    TYPE(obs_l), INTENT(inout) :: thisobs_l  !< Data type with local observation
    TYPE(obs_f), INTENT(inout) :: thisobs    !< Data type with full observation
    REAL(kind=8), INTENT(in) :: coords_l(:)          !< Coordinates of current water column (thisobs%ncoord)
    REAL(kind=8), INTENT(in) :: lradius              !< Localization radius in meters
...
! *** Arguments ***
    TYPE(obs_l), INTENT(inout) :: thisobs_l  !< Data type with local observation
    TYPE(obs_f), INTENT(inout) :: thisobs    !< Data type with full observation
    REAL(kind=8), INTENT(in) :: obs_f_all(:)         !< Full obs. vector of current obs. for all variables
    REAL(kind=8), INTENT(inout) :: obs_l_all(:)      !< Local observation vector for all variables
...
! *** Arguments ***
    ...
    REAL(kind=8), INTENT(in)    :: lradius        !< localization radius
    REAL(kind=8), INTENT(in)    :: sradius        !< support radius for weight functions
    REAL(kind=8), INTENT(in)    :: coords(:,:)    !< Coordinates of state vector elements
    REAL(kind=8), INTENT(inout) :: HP(:, :)       !< Matrix HP, dimension (nobs, dim)
    REAL(kind=8), INTENT(inout) :: HPH(:, :)      !< Matrix HPH, dimension (nobs, nobs)
...
! *** Arguments ***
    TYPE(obs_l), INTENT(inout) :: thisobs_l  !< Data type with local observation
    REAL(kind=8), INTENT(in) :: obs_f_one(:)         !< Full obs. vector of current obs. type (nobs_f_one)
    REAL(kind=8), INTENT(inout) :: obs_l_all(:)      !< Local observation vector for all variables (nobs_l_all)
...
! *** Arguments ***
    TYPE(obs_f), INTENT(in) :: thisobs    !< Data type with full observation
    REAL(kind=8), INTENT(in) :: coordsA(:)           !< Coordinates of current analysis domain (ncoord)
    REAL(kind=8), INTENT(in) :: coordsB(:)           !< Coordinates of observation (ncoord)
...
! *** Arguments ***
    ...
    REAL(kind=8), INTENT(in)    :: sradius        !< support radius for weight functions
    REAL, INTENT(in)    :: matA(:,:)      !< 
    REAL(kind=8), INTENT(in)    :: ivar_obs_l(:)  !< Local vector of inverse obs. variances (nobs_l)
    REAL(kind=8), INTENT(in)    :: dist_l(:)      !< Local vector of obs. distances (nobs_l)
