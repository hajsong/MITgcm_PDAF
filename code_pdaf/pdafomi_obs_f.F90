...
! *** Data type to define the full observations by internally shared variables of the module
  STRUCTURE /obs_f/
     ...
     REAL*8, ALLOCATABLE :: obs_f(:)        !< Full observed field
...
! *** Arguments ***
    ...
    REAL*8, INTENT(in) :: obs_p(:)            !< Vector of process-local observations
    REAL*8, INTENT(in) :: ivar_obs_p(:)       !< Vector of process-local inverse observation error variance
    REAL*8, INTENT(in) :: ocoord_p(:,:)       !< Array of process-local observation coordinates
    INTEGER, INTENT(in) :: ncoord           !< Number of rows of coordinate array
    REAL(8), INTENT(in) :: lradius             !< Localization radius (the maximum radius used in this process domain) 
    INTEGER, INTENT(out) :: dim_obs_f       !< Full number of observations

! *** Local variables ***
    INTEGER :: i                            ! Counter
    REAL*8, ALLOCATABLE :: obs_g(:)           ! Global full observation vector (used in case of limited obs.)
    REAL*8, ALLOCATABLE :: ivar_obs_g(:)      ! Global full inverse variances (used in case of limited obs.)
    REAL*8, ALLOCATABLE :: ocoord_g(:,:)      ! Global full observation coordinates (used in case of limited obs.)
...
! *** Arguments ***
    ...
    REAL*8, INTENT(in) :: obsstate_p(:)      !< Vector of process-local observed state
    REAL*8, INTENT(inout) :: obsstate_f(:)   !< Full observed vector for all types

! *** Local variables ***
    INTEGER :: status                      ! Status flag for PDAF gather operation
    INTEGER :: localfilter                 ! Whether the filter is domain-localized
    REAL*8, ALLOCATABLE :: obsstate_tmp(:)   ! Temporary vector of globally full observations
...
! *** Arguments ***
    ...
    REAL*8, INTENT(in) :: A_p(:, :)         !< Input matrix (nobs_f, ncols)
    REAL*8, INTENT(out)   :: C_p(:, :)      !< Output matrix (nobs_f, ncols)
...
! *** Arguments ***
    ...
    REAL(8), INTENT(in) :: lradius            !< Localization radius (used is a constant one here)
    REAL*8, INTENT(in) :: oc_f(:,:)          !< observation coordinates (radians), row 1: lon, 2: lat
                                           !< ranges: longitude (-pi, pi), latitude (-pi/2, pi/2)
...
! *** Arguments ***
    TYPE(obs_f), INTENT(inout) :: thisobs  !< Data type with full observation
    REAL*8, INTENT(in) :: obs_f_one(:)       !< Global full observation vector (nobs_f)
    REAL*8, INTENT(out) :: obs_f_lim(:)      !< full observation vector for process domains (nobs_lim)
...
! !ARGUMENTS:
  ...
  REAL*8, INTENT(in)  :: obs_p(:)  ! PE-local vector
  REAL*8, INTENT(out) :: obs_f(:)  ! Full gathered vector
...
! !ARGUMENTS:
  ...
  REAL*8, INTENT(in)  :: coords_p(:,:)  ! PE-local array
  REAL*8, INTENT(out) :: coords_f(:,:)  ! Full gathered array
  INTEGER, INTENT(out) :: status   ! Status flag: (0) no error
