...
!
!$Id: PDAFomi_obs_op.F90 941 2021-12-09 14:28:03Z lnerger $
...
! *** Arguments ***
    ...
    REAL(kind=8), INTENT(in)    :: state_p(:)      !< PE-local model state (dim_p)
    REAL(kind=8), INTENT(inout) :: obs_f_all(:)    !< Full observed state for all observation types (nobs_f_all)

! *** Local variables ***
    INTEGER :: i                           ! Counter
    REAL(kind=8), ALLOCATABLE :: ostate_p(:)       ! local observed part of state vector
